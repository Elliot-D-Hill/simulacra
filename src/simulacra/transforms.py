...
def fixed_effects(data: CovariateData, k: int, beta: Prior) -> PredictorData:
    *batch, _, _, p = data.X.shape
    ...
    return PredictorData(
        X=data.X, coordinates=data.coordinates,
        eta=eta, beta=coefficient.squeeze(-3),
    )
...
def random_effects(
    ...
    *batch, n, t, k = data.eta.shape
    ...
    return replace(data, eta=data.eta + eta, random_effect=(
        *data.random_effect, RandomEffect(W=membership, B=basis, b=coefficient),
    ))
...
def projection(data: PredictorData, output: int, weight: Prior) -> PredictorData:
    *batch, _, _, k_in = data.eta.shape
    ...
    return replace(data, eta=data.eta @ w, projection_weight=(
        *data.projection_weight, w.squeeze(-3),
    ))
